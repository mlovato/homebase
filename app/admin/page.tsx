    ...
    fetch("/api/settings")
      ...
      .then((data) => {
        ...
        if (interval && HEALTH_CHECK_INTERVALS.includes(interval)) {
          setIntervalMs(
            INTERVAL_TO_MS[interval as keyof typeof INTERVAL_TO_MS],
          );
        }
      })
      .catch(() => {});
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-gray-200 dark:border-gray-700 retro:border-retro-dim bg-white dark:bg-gray-800 retro:bg-retro-bg px-4 md:px-6 py-3 md:py-4 flex items-center justify-between shrink-0">
        ...
        <NextLink
          href="/"
          className="flex items-center gap-2 text-sm text-gray-500 retro:text-retro-dim hover:text-indigo-600 retro:hover:text-retro-green transition-colors"
        >
          {currentUser && (
            <UserAvatar
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          className={`fixed inset-y-0 left-0 z-50 w-52 bg-white dark:bg-gray-800 retro:bg-retro-bg border-r border-gray-200 dark:border-gray-700 retro:border-retro-dim p-4 pt-20 flex flex-col gap-1 transition-transform duration-200 md:static md:z-auto md:translate-x-0 md:pt-4 md:shrink-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
          {navItem(
            "links",
            "Links",
          ...
          {error && (
            <div className="mb-6 flex items-center justify-between gap-4 px-4 py-3 rounded-lg bg-red-50 dark:bg-red-900/20 retro:bg-transparent retro:border retro:border-retro-green border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 retro:text-retro-green text-sm">
              ...
              <button
                ...
                className="shrink-0 text-red-400 hover:text-red-600 retro:text-retro-dim retro:hover:text-retro-green"
              >
                ✕
              </button>
            </div>
