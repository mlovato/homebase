...
describe('export handler', () => {
  ...
  it('includes categories with their links, without DB ids', () => {
    ...
    createLink(db, { category_id: cat.id, name: 'Plex', url: 'http://plex', icon_type: 'builtin', icon_value: 'plex', sort_order: 0 })

    const result = handleExport(db)
    expect(result.categories).toHaveLength(1)
    expect(result.categories[0]).toMatchObject({ name: 'Media', sort_order: 0 })
    expect(result.categories[0]).not.toHaveProperty('id')
    expect(result.categories[0].links).toHaveLength(1)
    expect(result.categories[0].links[0]).toMatchObject({ name: 'Plex', url: 'http://plex', icon_type: 'builtin', icon_value: 'plex' })
  ...
  it('includes uncategorized links without DB ids', () => {
    const db = createTestDb()
    createLink(db, { category_id: null, name: 'Misc', url: 'http://misc', icon_type: 'url', icon_value: 'http://icon', sort_order: 1 })

    const result = handleExport(db)
    expect(result.uncategorized).toHaveLength(1)
    expect(result.uncategorized[0]).toMatchObject({ name: 'Misc', url: 'http://misc', sort_order: 1 })
