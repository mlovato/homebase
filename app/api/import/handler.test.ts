...
import { createCategory, getCategoriesWithLinks, getUncategorizedLinks } from '@/lib/repositories/categories'
...
describe('import handler', () => {
  it('returns 400 for missing version', () => {
    const result = handleImport(db, userId, { categories: [], uncategorized: [] })
  ...
  it('replaces all existing data for this user', () => {
    ...
    createLink(db, userId, { category_id: cat.id, name: 'OldLink', url: 'http://old', icon_type: 'builtin', icon_value: null, sort_order: 0 })
  ...
  it('handles import with no categories or links', () => {
    const result = handleImport(db, userId, { version: 1, exported_at: '', categories: [], uncategorized: [] })
    expect(result.status).toBe(200)
