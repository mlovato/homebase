...
import { getAllUsers, createUser, updateUser, deleteUser, getUserByEmail } from '@/lib/repositories/users'
...
export async function handleCreateUser(
  ...
  if (body.avatar && !AVATAR_OPTIONS.includes(body.avatar as typeof AVATAR_OPTIONS[number])) {
...
export async function handleUpdateUser(
  db: Database.Database,
  id: number,
  body: { email?: string; password?: string; role?: string; avatar?: string | null }
) {
  ...
  const updates: { email?: string; password_hash?: string; role?: UserRole; avatar?: string | null } = {}
