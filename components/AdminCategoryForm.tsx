...
export function AdminCategoryForm({ initialName, existingNames = [], onSubmit, onCancel }: AdminCategoryFormProps) {
  const [name, setName] = useState(initialName ?? '')
  const isEdit = initialName !== undefined

  const isDuplicate = name.trim() !== '' && existingNames.some(
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        ...
        />
        {isDuplicate && (
          <p className="text-sm text-red-500">A category with the same name already exists</p>
        )}
      </div>
