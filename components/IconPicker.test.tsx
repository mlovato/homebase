...
describe("IconPicker", () => {
  it("renders three tabs: Built-in, Upload, URL", () => {
    ...
    expect(
      screen.getByRole("button", { name: /built-in/i }),
    ).toBeInTheDocument();
  ...
  it("shows icon preview when a slug is set", () => {
    render(
      <IconPicker
        value={{ icon_type: "builtin", icon_value: "plex" }}
        onChange={noOp}
        serviceName=""
      />,
    );
    expect(screen.getByAltText("plex")).toHaveAttribute(
      "src",
      expect.stringContaining("plex.svg"),
    );
  });
  ...
  it("shows suggestion dropdown when search returns results", async () => {
    ...
    render(<IconPicker value={defaultValue} onChange={noOp} serviceName="" />);
    fireEvent.change(screen.getByPlaceholderText(/search icon/i), {
      target: { value: "pl" },
    });
  ...
  it("calls onChange with selected slug when suggestion is clicked", async () => {
    ...
    render(
      <IconPicker value={defaultValue} onChange={onChange} serviceName="" />,
    );
    fireEvent.change(screen.getByPlaceholderText(/search icon/i), {
      target: { value: "pl" },
    });
  ...
  it("calls onChange when url tab value is entered", () => {
    ...
    render(
      <IconPicker
        value={{ icon_type: "url", icon_value: null }}
        onChange={onChange}
        serviceName=""
      />,
    );
