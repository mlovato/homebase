...
export function IconPicker({
  ...
  const [urlValue, setUrlValue] = useState(
    value.icon_type === "url" ? (value.icon_value ?? "") : "",
  );
  ...
  const { suggestions, failed: searchFailed } = useIconSearch(searchQuery);
  ...
  function switchTab(newTab: Tab) {
    setTab(newTab);
    if (newTab === "builtin")
      onChange({ icon_type: "builtin", icon_value: slug.trim() || null });
    if (newTab === "upload")
      ...
      {tab === "builtin" && (
        <div className="flex flex-col gap-2">
          <div className="relative" ref={dropdownRef}>
            ...
            />
            {searchFailed && (
              <p className="text-xs text-amber-600 dark:text-amber-400 retro:text-retro-dim">
                Icon search is unavailable right now.
              </p>
            )}
          ...
          <div className="flex items-center gap-3 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/40 min-h-[52px]">
            {slug.trim() ? (
              ...
              <span className="text-xs text-gray-400 dark:text-gray-500">
                Icon preview — type a name above or search
              </span>
            )}
          </div>
        </div>
      ...
      {tab === "url" && (
        <input
          ...
          onChange={(e) => {
            ...
            onChange({
              icon_type: "url",
              icon_value: e.target.value.trim() || null,
            });
          }}
