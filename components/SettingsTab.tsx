...
const actionButtonClass =
  "px-4 py-2 rounded-lg retro:rounded-none border-2 border-gray-200 dark:border-gray-600 retro:border-retro-dim text-sm font-medium text-gray-600 dark:text-gray-300 retro:text-retro-green hover:border-indigo-400 dark:hover:border-indigo-500 retro:hover:border-retro-green transition-colors";
...
export function SettingsTab({ onIntervalChange }: SettingsTabProps = {}) {
  ...
  useEffect(() => {
    fetch("/api/settings")
      ...
      .then((data) => {
        ...
        if (data.search_shortcut && isValidShortcut(data.search_shortcut))
          setShortcut(data.search_shortcut);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="max-w-lg">
      <h2 className="text-xl font-semibold retro:text-retro-green mb-8">
        Settings
      </h2>
      ...
      <section className="mb-10">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-600 dark:text-gray-300 retro:text-retro-green w-28">
            Open search
          </span>
      ...
      <section className="mb-10">
        ...
        <p className="mt-3 text-xs text-gray-400 dark:text-gray-500 retro:text-retro-dim">
          Export saves all links and categories to a JSON file. Import replaces
          all existing data.
        </p>
      </section>
      ...
      <section className="mt-10">
        ...
        >
          ...
          {pwMsg && (
            <p
              className={`text-xs ${pwMsg.ok ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"} retro:text-retro-green`}
            >
              {pwMsg.text}
            </p>
          )}
