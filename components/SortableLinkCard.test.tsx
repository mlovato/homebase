...
describe('SortableLinkCard', () => {
  it('renders the link name', () => {
    render(<SortableLinkCard link={link} onEdit={jest.fn()} onDelete={jest.fn()} intervalMs={10000} />)
  ...
  it('shows status dot when intervalMs is a number', () => {
    render(
      <HealthCheckContext.Provider value={{ 'http://plex.local': 'up' }}>
        <SortableLinkCard link={link} onEdit={jest.fn()} onDelete={jest.fn()} intervalMs={10000} />
  ...
  it('hides status dot when intervalMs is null', () => {
    render(<SortableLinkCard link={link} onEdit={jest.fn()} onDelete={jest.fn()} intervalMs={null} />)
