  ...
  if (loading) {
    return (
      <div className="max-w-2xl">
        ...
        <p className="text-gray-500 dark:text-gray-400 retro:text-retro-dim text-sm">
          Loading…
        </p>
      </div>
    );
        ...
        <table className="w-full text-sm min-w-0">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-700/50 retro:bg-transparent border-b border-gray-200 dark:border-gray-700 retro:border-retro-dim">
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400 retro:text-retro-dim">
                User
              </th>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400 retro:text-retro-dim">
                Role
              </th>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400 retro:text-retro-dim hidden sm:table-cell">
                Created
              </th>
              <th className="text-right px-4 py-3 font-medium text-gray-500 dark:text-gray-400 retro:text-retro-dim">
                Actions
              </th>
            </tr>
          </thead>
          <tbody>
            {users.map((user) => (
              <tr
                key={user.id}
                className="border-b last:border-b-0 border-gray-200 dark:border-gray-700 retro:border-retro-dim"
              >
            ...
            {users.length === 0 && (
              <tr>
                <td
                  colSpan={4}
                  className="px-4 py-8 text-center text-gray-400 dark:text-gray-500 retro:text-retro-dim"
                >
                  No users yet
                </td>
              </tr>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        showError(
          data.error ?? `Failed to ${isEdit ? "update" : "create"} user`,
        );
        return;
      }
      onSaved();
