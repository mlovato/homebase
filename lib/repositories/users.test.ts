...
import { createUser, getUserById, getUserByEmail, getAllUsers, updateUser, deleteUser } from './users'
...
describe('createUser', () => {
  it('creates a user and returns it', () => {
    const user = createUser(db, { email: 'alice@test.com', password_hash: 'hash123' })
  ...
  it('creates an admin user when role is specified', () => {
    const user = createUser(db, { email: 'admin@test.com', password_hash: 'hash', role: 'admin' })
...
describe('getUserById', () => {
  it('returns the user', () => {
    const created = createUser(db, { email: 'bob@test.com', password_hash: 'hash' })
...
describe('updateUser', () => {
  it('updates email', () => {
    const user = createUser(db, { email: 'old@test.com', password_hash: 'hash' })
  ...
  it('updates password_hash', () => {
    const user = createUser(db, { email: 'a@test.com', password_hash: 'old_hash' })
...
describe('deleteUser', () => {
  it('deletes the user', () => {
    const user = createUser(db, { email: 'del@test.com', password_hash: 'hash' })
